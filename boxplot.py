        ...
        def gen_data():
            for val in vals:
                for _ in range(repeats):
                    ...
                    f = 300 + np.random.rand() * 2700
                    f3 = f if case == 1 else 300 + np.random.rand() * 2700

                    x1, x2, x3, _ = get_random_scene(f, f, f3, 200, dominant_plane=val)
...
def noise_box_plot(case=1, dominant_portion=0.5, load=True, repeats=100, legend_visible=True):
    ...
    for _ in range(repeats):
        f = 300 + np.random.rand() * 2700
        f3 = f if case == 1 else 300 + np.random.rand() * 2700
        x1, x2, x3, _ = get_random_scene(f, f, f3, 300, dominant_plane=dominant_portion)
